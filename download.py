...
class YoutubeDownloader:
    ...
    def _download_pytube(url: str, folder: str) -> bool:
        try:
            ...
            print("Download using pytube failed")
            return False
    ...
    def _download_ytdl(url: str, folder: str) -> bool:
        try:
            ...
            print("Download using youtube_dl failed")
            return False
...
class DownloadManager:
    def __init__(
        self,
        default_number_videos: int = 3,
        target_audio_folder: str = "mp3s",
        target_video_folder: str = "videos",
        tmp_folder: str = "tmp",
    ) -> None:
        self.save_audio: bool = (
            self._get_input("Do you want to extract the audio from downloaded videos ? yes / no : ", ["yes", "no"])
            == "yes"
        )
        print("not " * (not self.save_audio) + "saving audio\n")

        self.search_urls: bool = (
            self._get_input("Do you want to search for videos or input URLs ? search / url : ", ["search", "url"])
            == "url"
            )
        print()
    ...
    def run(self):
        while True:
            if self.search_urls:
                answer = input('Enter a youtube URL to download. Type "quit" to exit program. Type "search" to switch to search mode\n')
                if answer.lower() == "quit":
                    break

                if answer.lower() == "search":
                    self.search_urls = False
                    continue

                self._download_via_url(answer)
            else:
                answer = input('Enter a query (i.e. list of keywords) to search on youtube. Type "quit" to exit program. Type "url" to switch to URL mode\n')
                if answer.lower() == "quit":
                    break

                if answer.lower() == "url":
                    self.search_urls = True
                    continue
    ...
    def _download_via_url(self, url: str):
        info = YoutubeScraper.get_video_info(url)
        print(f"""\nDownloading {info["Title"]}, from {info["Author"]}...""")
        YoutubeDownloader.download(url, self.tmp_folder)
        ...
        while True:
            ...
            if len(video_links) == 0:
                answer = self._get_input(
                    "No videos were found. Type 'cancel' to cancel. Type 'restart' to search for videos again with more loading time for the browser. Type 'url' to manually enter a URL to download\n",
                    ["cancel", "restart", "url"] + [f"{i+1}" for i in range(n_videos)],
                )
            elif len(video_links) < n_videos:
                answer = self._get_input(
                    "Type in the number of the video you want to download, or type 'cancel' to cancel. Type 'restart' to search for videos again with more loading time for the browser. \n",
                    ["cancel", "restart"] + [f"{i+1}" for i in range(n_videos)],
                )
            else:
                answer = self._get_input(
                    "Type in the number of the video you want to download, type 'more' to see more options, or type 'cancel' to cancel. \n",
